...
//
// Les tracés de la clé de sol, du bémol et du dièse viennent de Material
// Design Icons (open source) plutôt que d'être dessinés à la main, pour
// qu'ils soient nets et reconnaissables au lieu de vagues gribouillis.
const CHEMIN_CLEF_SOL = "M13 11V7.5l2.2-2.21c.8-.79.95-2.05.39-3.03A2.47 2.47 0 0 0 13.45 1c-.21 0-.45.03-.64.09C11.73 1.38 11 2.38 11 3.5v3.24L7.86 9.91a5.95 5.95 0 0 0-1.25 6.43c.77 1.9 2.45 3.21 4.39 3.55v.61c0 .26-.23.5-.5.5H9v2h1.5c1.35 0 2.5-1.11 2.5-2.5V20c2.03 0 4.16-1.92 4.16-4.75c0-2.3-1.92-4.25-4.16-4.25m0-7.5c0-.23.11-.41.32-.47c.22-.06.45.03.56.23a.5.5 0 0 1-.08.61l-.8.86zm-2 8c-.97.64-1.7 1.74-1.96 2.76l1.96.52v3.05a3.86 3.86 0 0 1-2.57-2.26c-.59-1.46-.27-3.12.83-4.24L11 9.5zm2 6.5v-5.06c1.17 0 2.18 1.1 2.18 2.31C15.18 17 13.91 18 13 18";
const CHEMIN_BEMOL = "M8.5 19c4.86-2.74 7-5.09 7-7c0-1.41-.71-3-3-3c-.7 0-1.39.28-2 .67V5h-2m2 10.38v-3.12c.62-.67 1.45-1.26 2-1.26c.59 0 1 .07 1 1c0 .15-.1 1.3-3 3.38";
const CHEMIN_DIESE = "M17 9.5v-2l-2 .6V5.5h-2v3.2l-2 .6V6.5H9v3.4l-2 .6v2l2-.6v2l-2 .6v2l2-.6v2.6h2v-3.2l2-.6v2.8h2v-3.4l2-.6v-2l-2 .6v-2zm-4 3.2l-2 .6v-2l2-.6z";

function noteSvg(x, y, classe) {
    return `<g transform="translate(${x},${y})"><g class="note ${classe}"><ellipse cx="0" cy="0" rx="6.3" ry="4.8" transform="rotate(-18)"></ellipse><line x1="6" y1="-1.5" x2="6" y2="-24"></line></g></g>`;
}

function crochesLieesSvg(x, y, classe) {
    return `<g transform="translate(${x},${y})"><g class="note ${classe}"><ellipse cx="0" cy="0" rx="6.3" ry="4.8" transform="translate(0,9) rotate(-18)"></ellipse><line x1="6" y1="7.5" x2="6" y2="-15"></line><ellipse cx="0" cy="0" rx="6.3" ry="4.8" transform="translate(33,15) rotate(-18)"></ellipse><line x1="39" y1="13.5" x2="39" y2="-9"></line><line x1="6" y1="-15" x2="39" y2="-9" class="barre-croches"></line></g></g>`;
}

function etincelleSvg(x, y, classe) {
    return `<g transform="translate(${x},${y})"><path class="etincelle ${classe}" d="M0,-4.5 L1.2,-1.2 L4.5,0 L1.2,1.2 L0,4.5 L-1.2,1.2 L-4.5,0 L-1.2,-1.2 Z"></path></g>`;
}

// accidental: "" (aucun) | "bemol" | "diese"
function construirePortee(largeur, accidental, elementsHtml) {
    const accidentalSvg = accidental === "bemol"
        ? `<g class="bemol" transform="translate(58,8) scale(1.2)"><path d="${CHEMIN_BEMOL}"></path></g>`
        : accidental === "diese"
        ? `<g class="diese" transform="translate(58,7.5) scale(1.15)"><path d="${CHEMIN_DIESE}"></path></g>`
        : "";

    return `<div class="portee-musicale" aria-hidden="true">
    <svg viewBox="0 -15 ${largeur} 75" class="portee-svg" style="width:${largeur}px;height:75px;">
        <g class="lignes-portee"><line x1="51" y1="9" x2="${largeur}" y2="9"></line><line x1="51" y1="18" x2="${largeur}" y2="18"></line><line x1="51" y1="27" x2="${largeur}" y2="27"></line><line x1="51" y1="36" x2="${largeur}" y2="36"></line><line x1="51" y1="45" x2="${largeur}" y2="45"></line></g><g class="clef-sol" transform="translate(5,-11) scale(2.7)"><path d="${CHEMIN_CLEF_SOL}"></path></g>${accidentalSvg}${elementsHtml}
    </svg>
</div>`;
}

const PORTEES_MUSICALES = {
    paroles: construirePortee(300, "diese",
        noteSvg(80, 39, "note-1") + noteSvg(150, 27, "note-2") + noteSvg(220, 15, "note-4") +
        noteSvg(280, 6, "note-5") + etincelleSvg(286, -3, "etincelle-1")),

    chant: construirePortee(190, "",
        noteSvg(90, 27, "note-2") + noteSvg(150, 18, "note-5")),

    galerie: construirePortee(320, "bemol",
        noteSvg(85, 39, "note-4") + noteSvg(150, 15, "note-5") + noteSvg(210, 15, "note-2") +
        noteSvg(270, 39, "note-1") + etincelleSvg(115, 3, "etincelle-1") + etincelleSvg(300, 3, "etincelle-2")),

    evenements: construirePortee(300, "diese",
        crochesLieesSvg(90, 27, "note-paire") + crochesLieesSvg(210, 18, "note-paire2")),

    hierarchie: construirePortee(330, "bemol",
        noteSvg(80, 39, "note-1") + noteSvg(135, 33, "note-2") + noteSvg(190, 27, "note-4") +
        noteSvg(245, 21, "note-5") + noteSvg(300, 15, "note-paire2") + etincelleSvg(306, 3, "etincelle-1")),
};
